    ...
    init = function(player, options) {
      ...
      var moveListener = function(event) {
        ...
        if (!player.hasClass('vjs-fullscreen')){
          var playerDimensions = player.currentDimensions();
          var playerWidthPercentage = playerDimensions.width / screen.width;
          var playerHeightPercentage = playerDimensions.height / screen.height;
    
           if (setting.style) {
            img.style.left = parseFloat(setting.style.left)*playerWidthPercentage+'px';
            img.style.top = parseFloat(setting.style.top)*playerHeightPercentage+'px';
          }
          if (img.naturalWidth) {
            img.style.width = img.naturalWidth*playerWidthPercentage+'px';
            img.style.height = img.naturalHeight*playerHeightPercentage+'px';
          }

          width = parseFloat(setting.width || settings[0].width)*playerWidthPercentage;
          height = parseFloat(setting.height || settings[0].height)*playerHeightPercentage;
        }
        else{
          if (setting.style) {
            img.style.left = parseFloat(setting.style.left)+'px';
            img.style.top = parseFloat(setting.style.top)+'px';
          }
          if (img.naturalWidth) {
          ...
          width = parseFloat(setting.width || settings[0].width);
          height = parseFloat(setting.height || settings[0].height);
        }
