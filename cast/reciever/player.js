...
sampleplayer.CastPlayer.prototype.loadVideo_ = function (info) {
	...
	if (!protocolFunc) {
		...
		if (!this.preloadPlayer_ || (this.preloadPlayer_.getHost &&
			...
			//Override the processing of the licence request
			host.updateLicenseRequestInfo = function (reqInfo) {
				reqInfo.headers["nv-authorizations"] = info.message.media.customData.rights;
				reqInfo.headers["nv-tenant-id"] = "CON935WN";
				if (reqInfo.protectionSystem === cast.player.api.ContentProtection.WIDEVINE) {
					reqInfo.headers.Accept = "application/json";
					reqInfo.headers["content-type"] = "application/json";
					reqInfo.url = info.message.media.customData.WVLicenceUrl || host.licenseUrl;
					reqInfo.content = OTV.ssp.SSPLicenseFormatter.packagePayload(reqInfo.content);

				} else {
					reqInfo.url = info.message.media.customData.PRLicenceUrl || host.licenseUrl;
				}
			};

			//Override the processing of the licence response
			host.processLicense = OTV.ssp.SSPLicenseFormatter.unpackageLicense;
